...
vector<double> NeuralNetwork::predict(vector<double> values) {
    ...
    for (int indexPeso = 0; indexPeso < setDePesos; indexPeso++) {
        Matrix tempResults = Matrix::multiply(this->pesos[indexPeso], inputForLayer);
        tempResults.addScalar(this->biases[indexPeso].toArray()[0]);
        tempResults.sigmoid();
        layerOutputs[indexPeso] = tempResults;
...
void NeuralNetwork::trainNuevo(vector<double> entrada, vector<double> expectedOutputValue, int epochs) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        hiddenLayerErrors.multiply(error.toArray()[0]);

        layerGradient = layerOutputs[0].dsigmoid();
        layerGradient.multiply(hiddenLayerErrors.toArray()[0] /*weightsNextLayerTransposed*/);
        layerGradient.multiply(this->learningRate);
